			...
			<div class="wrap-table100">
				...
				<div class="input-group row" style="margin:0px 20px 20px 0px;">						
						...
						<div style="border:1px solid gray;margin-left:250px;width:500px; height:">					
							...
							<button type="button" style="margin-left:20px;background:crimson;color:white;" class="btn btn-lg btn-crimson" onclick="OpenWindow('<%=request.getContextPath()%>/facility_stock/facility_supply_management/regist','','600','800');">	
							<b>물품등록</b>
							</button>
					...
					<table>
						...
						<c:forEach items="${itemList}" var="itemList">
								<tr>
									<td class="column1"><a onclick="OpenWindow('<%=request.getContextPath()%>/facility_stock/facility_supply_management/detail?item_code=${itemList.item_code}','','850','620');">${itemList.item_code}</a></td>
									<td class="column2">${itemList.item_name}</td>
									<td class="column3">${itemList.item_price}</td>
									<td class="column4">${itemList.item_cnt}</td>
									<td class="column5">${itemList.facility_name}</td>
									<td class="column6"><fmt:formatDate value="${itemList.regdate}" pattern="yyyy-MM-dd" /></td>
								</tr>
						</c:forEach>		
						</tbody>
					</table>
