	...
	<body id="page-top" style="height: 700px;width: 405px;">
	  ...
	  <nav class="navbar navbar-expand-lg navbar-light fixed-top" id="mainNav">
	    <div class="container">
	    	...
	    	<span style="font-weight: bold; font-size:20px;">나의 납부내역</span>
					...
					<ul class="list-group" style="width:405px; margin-left:-12px;">
					  <c:forEach items="${paymentList}" var="payment">
					  	<c:set value="${loginUser}" var="loginUser"/>					  		
					  		<c:if test="${loginUser.member_code eq payment.member_code}">
					  			<li class="list-group-item" style="padding:18.9px;"><a href="<%=request.getContextPath()%>/payment/mobileDetail?payment_code=${payment.payment_code}" style="color:black;">납부 : <fmt:formatDate value="${payment.payment_date}" pattern="yyyy-MM-dd"/>(${payment.payment_type}) / 금액 : <fmt:formatNumber value="${payment.payment_amount}" type="currency"/>원</a></li>
					  		</c:if>
					  </c:forEach>
					</ul>
				</div>
