...
<form action="<%=request.getContextPath()%>/member/modifySuperintendent" method="post" id="modifyForm">
	<input type="hidden" name="member_code" value="${superintendent.member_code }">
    <div id="centerDiv" class="ibox-content no-padding border-left-right">
    <div id="leftinfor">
        <div class="ibox-content no-padding border-left-right">
        <br/>
            <IMG id="imgprofile" class="img-fluid" alt="image" src="<%=request.getContextPath()%>/member/getPicture?picture=${superintendent.member_picture }&member_code=${superintendent.member_code }" />
        </div>
            <h4><strong>${superintendent.member_name }</strong></h4>
            <p><i class="fa"></i>${superintendent.member_id} </p>
    </div>
        ...
        <div id="leftDiv" class="ibox-content profile-content">
           <h5>
            ...
            <p>
                	<label class="infor"><i class="fa fa-home"></i></label>${superintendent.member_address }<br/>
                	<label class="infor"><i class="fa fa-envelope"></i></label>${superintendent.member_email }<br/>
                	<label class="infor"><i class="fa fa-phone"></i></label>${superintendent.member_phone }<br/>
                	<label class="infor"><i class="fa fa-money"></i></label><input name="superintendent_pay" type="text" value="${superintendent.superintendent_pay }"><br/>
                	<label class="infor"><i class="fa fa-pencil"></i></label><input name="superintendent_significant" type="text" value="${superintendent.superintendent_significant }"><br/>
                	<label class="infor"><i class="fa fa-clock-o"></i></label>${superintendent.member_recent_login_time }<br/>
            </p>
...
$('#addFacility').on('click',function(){
	...
	var data = ${facilityJson};
	var html = template(data);
	var lenindex = $(".testlength").length;
	/* var phtml = "<select name='facility_codes[" + lenindex + "].facility_codes' class='testlength'>"; */
	var phtml = "<select name='facility_codes' class='testlength'>";
	var nhtml = "</select><i class='fa fa-minus-circle minusn'></i>";
	
	$(this).next('ul').append("<li>"+ phtml + html+ nhtml + "</li>");
	
})

$('#addTask').on('click',function(){
	
	var source = $('#select2-template').html();
	var template = Handlebars.compile(source);
	var data = ${taskListJson};
	var html = template(data);
	var lenindex = $(".testlength2").length;
	/* var phtml = "<select name='task_contentss[" + lenindex + "].task_contents' class='testlength2'>"; */
	var phtml = "<select name='task_codes' class='testlength2'>";
	var nhtml = "</select><i class='fa fa-minus-circle minusn'></i>";
