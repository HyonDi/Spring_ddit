...
<section class="content register-page" style="height: 586.391px; ">	<!-- form start -->
	...
	<div class="card" style="width: 600px;margin-left:120px;margin-top:20px">
		...
		<button type="button" id="modifyBtn" style="margin-bottom:30px;background:crimson;color:white;width:100px;float:right;margin:10px 10px 0px 10px" class="btn btn-lg btn-crimson">	
			...
			<form role="form" class="form-horizontal" action="<%=request.getContextPath()%>/facility_stock/facility_supply_management/regist" method="post" style="padding-top: 10px">
				...
				<div class="wrap-input100 p-r-10 ">
					<input readonly="readonly" class="input100 has-val" id="email" type="text" name="item_name" value="${item.item_name}">
					<span class="focus-input100"></span> <span class="label-input100">이름</span>
				</div>

				<div class="wrap-input100">
					<input readonly="readonly" class="input100 has-val" id="name" type="text" name="item_price" value="${item.item_price}">
					<span class="focus-input100"></span> <span class="label-input100">가격</span>
				</div>

				<div class="wrap-input100">
					<input readonly="readonly" class="input100 has-val" id="phone" type="text" name="item_cnt" value="${item.item_cnt}">
	...
	var imageURL = "<%=request.getContextPath()%>/facility_stock/facility_supply_management/getPicture?picture=${item.item_picture}";
	...
	$('#deleteBtn').on('click',function(e){
		...
		if(answer){
			...
			form.attr({
				"action":"<%=request.getContextPath()%>/facility_stock/facility_supply_management/delete",
				"method":"post"
			});
			form.submit();
		}
	...
	$('#modifyBtn').on('click',function(e){
		form.attr({
			"action":"<%=request.getContextPath()%>/facility_stock/facility_supply_management/modifyForm",
			"method":"post"
		});
		form.submit();
	});
