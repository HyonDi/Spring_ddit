	...
	<body id="page-top" style="height: 700px;width: 405px;">
	  ...
	  <nav class="navbar navbar-expand-lg navbar-light fixed-top" id="mainNav">
	    <div class="container">
	    	...
	    	<span style="font-weight: bold; font-size:20px;">나의 계약내역</span>
			...
			<div class="container">
					...
					<ul class="list-group" style="width:405px; margin-left:-12px;">
					  <c:forEach items="${constractList}" var="constract">
					  	<c:set value="${loginUser}" var="loginUser"/>					  		
					  		<c:if test="${loginUser.member_code eq constract.member_code}">
					  			<li class="list-group-item" style="padding:18.9px;"><a href="<%=request.getContextPath()%>/constract/mobileDetail?constract_code=${constract.constract_code}" style="color:black;">계약 : ${loginUser.member_address}(${constract.constract_type}) / 금액 : <fmt:formatNumber value="${constract.constract_amount}" type="currency"/>원</a></li>
					  		</c:if>
					  </c:forEach>
					</ul>
				</div>
				<nav aria-label="member list Navigation">
					...
					<form id="jobForm">
						...
						<input type="hidden" name="perPageNum" value="50">
