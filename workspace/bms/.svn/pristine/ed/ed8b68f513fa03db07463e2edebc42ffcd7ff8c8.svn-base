...
<body>
	<div class="titleDiv">
		<br/>
		<h1 id="title"> <b>To Do List 등록</b> </h1>
		<br/>
	</div>
	...
	<div class="ibox-content">
		...
		<div class="clockPick">
		    ...
		    <div class="form-group input-daterange input-group">
			          ...
			          <input id="startTime" class="form-control clock clockpicker2" type="text" placeholder="시작 시간">
			          <span class="input-group input-group-addon clock "> &emsp;~&emsp; </span>
			          <input id="endTime" class="form-control clock clockpicker2" type="text" placeholder="종료 시간">
			</div>
		</div>
		...
		<div class="sortCode">
			<h3>일정 분류</h3>
			<div class="form-group input-daterange input-group">
				<input class="form-control" id="newSortCode" type="text" placeholder="새로운 일정분류명을 입력해주세요." hidden="true"/>
	            <select class="form-control" id="sortCode"></select>
	
	            <input type='checkbox' id='makeSortCode' class="i-checks icheckbox_square-grey makeCheck" />
	            <label for='makeSortCode' onclick="ichk();">새 일정분류 만들기</label>
             </div>
		</div>
		<br/>
		<div id="buttonDiv" class="ibox-content">
		<br/>
			<button type="button" class="btn btn-outline btn-success">등&emsp;록</button>
...
/*  $(".icheckbox_square-grey").on('change','.iCheck-helper',function(e){
  ...
  $(document).ready(function(){
	  ...
	  //
// 	  alert($('#makeSortCode').prop("checked"));
